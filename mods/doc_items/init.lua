		...
		if data then
			...
			if not forbidden_core_factoids.node_movement then
				...
				if data.def.climbable == true then
					...
					elseif data.def.groups.disable_jump == 1 then
						datastring = datastring .. S("This block can be climbed, but only downwards.").."\n"
					elseif data.def.groups.disable_descend == 1 then
						datastring = datastring .. S("This block can be climbed, but only upwards.").."\n"
					else
...
TODO: If possible, try to replace this functionality by updating the revealed items as
      soon the player obtained a new item (probably needs new Luanti callbacks). ]]
local checktime = 8
local timer = 0
