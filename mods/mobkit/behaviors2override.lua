		...
		if tpos.y + self.height - pos.y > 1 then -- if head in next node above, else no point checking headroom
			...
			for p, node in pairs(nodes) do
				if snpos.x == p.x and snpos.z == p.z then
					...
					if node.name == "ignore" or (node.walkable and mobkit.get_node_height(p) > tpos.y + 0.001) then
						return
					end
				end
			end
...
function YOURNAMESPACE.get_next_waypoint(self, tpos)
	...
	if height and not liquidflag and not (nogopos and mobkit.isnear2d(pos2, nogopos, 0.1)) then
		...
		for i = 1, 3 do
			-- scan left
			local height, pos2, liq = YOURNAMESPACE.is_neighbor_node_reachable(
				self,
				YOURNAMESPACE.neighbor_shift(neighbor, -i * self.path_dir)
			)
			...
			-- scan right
			height, pos2, liq = YOURNAMESPACE.is_neighbor_node_reachable(
				self,
				YOURNAMESPACE.neighbor_shift(neighbor, i * self.path_dir)
			)
			if height and not liq and not (nogopos and mobkit.isnear2d(pos2, nogopos, 0.1)) then
		...
		--scan rear
		height, pos2, liquidflag =
			YOURNAMESPACE.is_neighbor_node_reachable(self, YOURNAMESPACE.neighbor_shift(neighbor, 4))
		if height and not liquidflag and not (nogopos and mobkit.isnear2d(pos2, nogopos, 0.1)) then
			update_pos_history(self, pos2)
			return height, pos2
		end
...
function YOURNAMESPACE.get_next_waypoint_fast(self, tpos, nogopos)
	...
	if height and not liquidflag then
		...
		for i = 1, 4 do
			-- scan left
			height, pos2, liq =
				YOURNAMESPACE.is_neighbor_node_reachable(self, YOURNAMESPACE.neighbor_shift(neighbor, -i))
...
function YOURNAMESPACE.hq_roam(self, prty)
	local func = function(self)
		if mobkit.is_queue_empty_low(self) and self.isonground then
			...
			if height and not liquidflag then
				YOURNAMESPACE.dumbstep(self, height, tpos, 0.3)
			end
		end
	end
	...
	local func = function(self)
		...
		if mobkit.is_queue_empty_low(self) and self.isonground then
			...
			if vector.distance(pos, opos) > 3 then
				...
				if height then
					YOURNAMESPACE.dumbstep(self, height, tpos)
				else
...
function YOURNAMESPACE.hq_aqua_attack(self, prty, tgtobj, speed)
	...
	local func = function(self)
		...
		local yaw = self.object:get_yaw()
		if mobkit.timer(self, 1) then
			if not YOURNAMESPACE.is_in_deep(tgtobj) then
				return true
			end
			local vel = self.object:get_velocity()
