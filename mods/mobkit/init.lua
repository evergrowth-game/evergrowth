...
function mobkit.pos_shift(pos, vec) -- vec components can be omitted e.g. vec={y=1}
	...
	vec.y = vec.y or 0
	vec.z = vec.z or 0
	return { x = pos.x + vec.x, y = pos.y + vec.y, z = pos.z + vec.z }
end
...
function mobkit.is_pos_in_box(pos, bpos, box)
	return pos.x > bpos.x + box[1]
		and pos.x < bpos.x + box[4]
		and pos.y > bpos.y + box[2]
		and pos.y < bpos.y + box[5]
		and pos.z > bpos.z + box[3]
		and pos.z < bpos.z + box[6]
end
...
function mobkit.dir_to_rot(v, rot)
	...
	return {
		x = (v.x == 0 and v.y == 0 and v.z == 0) and rot.x
...
function mobkit.get_box_displace_cols(pos, box, vec, dist)
	...
	-- displacement vector
	if dist then
		vec = vector.multiply(vector.normalize(vec), dist)
	end
...
function mobkit.is_alive(thing) -- thing can be luaentity or objectref.
	...
	if not mobkit.exists(thing) then
		return false
	end
	if type(thing) == "table" then
		return thing.hp > 0 or (thing.health and thing.health > 0)
...
function mobkit.exists(thing)
	...
	if type(thing) == "userdata" then
		if thing:is_player() then
			if thing:get_look_horizontal() then
				return true
			end
		else
			if thing:get_yaw() then
				return true
			end
		end
	end
...
function mobkit.drive_to_pos(self, tpos, speed, turn_rate, dist)
	local pos = self.object:get_pos()
	dist = dist or 0.2
	if mobkit.isnear2d(pos, tpos, dist) then
		return true
	end
...
function mobkit.timer(self, s) -- returns true approx every interval
...
function mobkit.get_nearby_entity(self, name) -- returns random nearby entity of name or nil
	for _, obj in ipairs(self.nearby_objects) do
		if mobkit.is_alive(obj) and not obj:is_player() and obj:get_luaentity().name == name then
			return obj
		end
	end
	return
...
function mobkit.physics(self)
	...
	if self.isonground and not self.isinliquid then
		vnew = vector.new(
			abs(vel.x) > 0.2 and vel.x * mobkit.friction or 0,
			vel.y,
...
function mobkit.actfunc(self, staticdata, dtime_s)
	...
	if
		self.timeout
		and (
			(self.timeout > 0 and dtime_s > self.timeout and next(self.memory) == nil)
			or (self.timeout < 0 and dtime_s > abs(self.timeout))
		)
	then
		self.object:remove()
	end
