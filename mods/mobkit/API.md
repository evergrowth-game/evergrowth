# Mobkit API Reference

1. [Concepts](#concepts)
   1. [Behavior functions](#behavior-functions)
      1. [Low level behavior functions](#low-level-behavior-functions)
      2. [High level behavior functions](#high-level-behavior-functions)
         1. [Priority](#priority)
      3. [Modifying built in behaviors](#modifying-built-in-behaviors)
   2. [Logic function](#logic-function)
   3. [Processing diagram](#processing-diagram)
   4. [Entity definition](#entity-definition)
   5. [Exposed luaentity members](#exposed-luaentity-members)

2. [Reference](#reference)
   1. [Utility functions](#utility-functions)
   2. [Built in behaviors](#built-in-behaviors)
      1. [High level behaviors](#high-level-behaviors)
      2. [Low level behaviors](#low-level-behaviors)
   3. [Constants and member variables](#constants-and-member-variables)
...
Queuing is covered by behavior defining conventions
...
hq_bhv1(self,priority):
...
hq_bhv2(self,priority) -- bhv1 kicks off bhv2 with equal priority
return true -- and ends,
-- bhv2 becomes active on the next engine step.

hq_bhv1(self,priority):
...
hq_bhv2(self,priority+1) -- bhv1 kicks off bhv2 with higher priority
...
#### Modifying built in behaviors

Do not modify example_behaviors.lua directly, because functions defined there are meant to be shared between mods.
Instead, copy the contents of /behaviors2override.lua into your mod/game, changing every occurrence of the string 'YOURNAMESPACE' to the name of a lua table representing your namespace of choice.
...
## Exposed luaentity members

Some frequently used entity fields to be accessed directly for convenience

```lua
    self.dtime    -- max(dtime as passed to on_step,0.5) - limit of 0.05 to prevent jerkiness on long steps.
...
function mobkit.is_there_yet2d(pos,dir,dest) -- obj position; facing vector; destination position
...
-- makes an assumption that luaentities are considered dead when their hp <= 0
-- thing can be luaentity or objectref.
...
> [!WARNING]
> Memory is saved when objects are unloaded, if the server crashes then memory will be forgotten!

```lua
function mobkit.remember(self,key,val)
-- permanently store a key, value pair
...
```lua
function mobkit.hq_roam(self,priority)
-- slow random roaming
-- never returns

function mobkit.hq_follow(self,priority,tgtobj)
-- follow the tgtobj
-- returns if tgtobj becomes inactive

function mobkit.hq_goto(self,priority,tpos)
-- go to tpos position
-- returns on arrival

function mobkit.hq_runfrom(self,priority,tgtobj)
-- run away from tgtobj object
-- returns when tgtobj far enough

function mobkit.hq_hunt(self,priority,tgtobj)
-- follow tgtobj and when close enough, kick off hq_attack
-- returns when tgtobj too far

function mobkit.hq_warn(self,priority,tgtobj)
...
function mobkit.hq_aqua_roam(self,priority,speed)
function mobkit.hq_aqua_attack(self,priority,tgtobj,speed)
function mobkit.hq_aqua_turn(self,priority,tyaw,speed)
...
self.isonground -- true if y velocity is 0 for at least two successive steps
